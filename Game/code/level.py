...
from tiles import Tile, StaticTile, Crate, Coin, Palm, Ship, Sail, Chest
...
class Level:
	def __init__(self,current_level,surface,create_overworld,change_coins,change_health):
		...
		if self.current_level == 4:
			...
			# sail
			sail_layout = import_csv_layout(level_data['sail'])
			self.sail_sprites = self.create_tile_group(sail_layout,'sail')
			...
			for col_index,val in enumerate(row):
				if val != '-1':
					...
					if type == 'fg palms':
						...
						if val == '1' or val == '2': sprite = Palm(tile_size,x,y,'../graphics/terrain/palm_large',64)

					if type == 'ship':
						sprite = Ship(tile_size,x,y,'../graphics/ship_enhanced',38)

					if type == 'sail':
						sprite = Sail(tile_size,x,y,'../graphics/sail',0,224)
					...
					if type == 'bg palms':
						...
						if type=='terrain':
							terrain_tile_list = import_cut_graphics('../graphics/ship/Terrain and Back Wall (64x64).png')
							tile_surface = terrain_tile_list[int(val)]
							sprite = StaticTile(tile_size,x,y,tile_surface)

						if type == 'platform':
							platform_tile_list = import_cut_graphics('../graphics/ship/Platforms (64x64).png')
							platform_surface = platform_tile_list[int(val)]
							sprite = StaticTile(tile_size,x,y,platform_surface)

						if type == 'bg':
							bg_tile_list = import_cut_graphics('../graphics/ship/Terrain and Back Wall (64x64).png')
	...
	def player_setup(self,layout,change_health):
		for row_index, row in enumerate(layout):
			for col_index,val in enumerate(row):
				...
				if val == '1':
					hat_surface = pygame.image.load('../graphics/skull/Skull.png').convert_alpha()
	...
	def horizontal_movement_collision(self):
		...
		if self.current_level < 4:
			...
			collidable_sprites = self.terrain_sprites.sprites() + self.crate_sprites.sprites() + self.platform_sprites.sprites() + self.wall_sprites.sprites()
		else:
			collidable_sprites = self.terrain_sprites.sprites() + self.crate_sprites.sprites() + self.fg_palm_sprites.sprites() + self.ship_sprites.sprites()
	...
	def check_enemy_collisions(self):
		enemy_collisons =  pygame.sprite.spritecollide(self.player.sprite, self.enemy_sprites, False)
		if enemy_collisons:
			for enemy in enemy_collisons:
	...
	def run(self):
		...
		if self.current_level == 4:
			# sail
			self.sail_sprites.update(self.world_shift)
			self.sail_sprites.draw(self.display_surface)
		...
		#self.ship_horizontal_collision()
		
		self.get_player_on_ground()
		self.vertical_movement_collision()
		#self.ship_vertical_collision()
